...
void setup() {
  ...
  // The baud rate set here will be ignored (because native USB)
  SERIAL.begin(1337);
...
void loop() {
  ...
  //static bool data_stream_requested = false;
  ...
  if /*((data_stream_requested && !data_ready)|*/(collect_data && !data_ready) {
    return;
  }

//  if(data_stream_requested){
//    //once the stream of data begins
//    collect_data = true;
//  }
  ...
  if (data_ready) {
    ...
    //indicates end of transmission for one dataset
    char stop_packet[5] = {0x86, 0, 0, 0, 0};
    SERIAL.write(stop_packet, 5);
  ...
  if (SERIAL.available() >= 5) {
    ...
    switch (opcode) {
      ...
      case 0x01: {
        ...
        // Queue the data to the DAC (for speakers)
